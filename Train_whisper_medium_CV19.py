...
#This Script trains Urdu CV-19 Ciroys finetuning the Pretrained Whisper Large model
...
# model = DataParallel(model, device_ids=[0,1,2,3]) #Lets see this change the memory issue
# model.to("cuda")

tokenizer = WhisperTokenizer.from_pretrained(model_name_or_path, language=language, task=task)

# labels = tokenizer(input_str).input_ids
# tokenized_input=tokenizer(input_str, return_tensors="pt", padding=True, truncation=True)
...
#common_voice["train"]= load_dataset("csv", data_files={"train": "/data3/sharif/Datasets/cv-corpus-19.0-2024-09-13/ur/train.tsv"}, delimiter="\t")["train"]

print("--------------Adding two splits Train and Validated--------------")
train_split = load_dataset("csv", data_files={"/data3/sharif/Datasets/cv-corpus-19.0-2024-09-13/ur/train.tsv"}, delimiter="\t")["train"]
validated_split = load_dataset("csv", data_files={"/data3/sharif/Datasets/cv-corpus-19.0-2024-09-13/ur/validated.tsv"}, delimiter="\t")["train"]

train_split = train_split.remove_columns(
    ["accents", "age", "client_id", "sentence_domain", "down_votes", "gender", "locale", "segment", "up_votes"]
)
validated_split = validated_split.remove_columns(
    ["accents", "age", "client_id", "sentence_domain", "down_votes", "gender", "locale", "segment", "up_votes"]
)

print(train_split)
print(validated_split)
combined = concatenate_datasets([train_split, validated_split]) #Combined the Train and Validated Splits 
common_voice = DatasetDict()
common_voice["train"] =combined
...
#     output_dir="./whisper-LARGE-urdu_CV-19",  # REPO: its actually pointing the whisper small pretrained model
...
# )
training_args = Seq2SeqTrainingArguments(
    output_dir="./whisper-LARGE-urdu_CV-19",  # change to a repo name of your choice
    per_device_train_batch_size=4,  #changed from 8
    gradient_accumulation_steps=16,  # 8 prev: increase by 2x for every 2x decrease in batch size
    learning_rate=1e-3,
    warmup_steps=50,
    num_train_epochs=3,
    max_steps=12,
    ...
    fp16=True, #To use AMP, Mixed Precision Training
    #num_gpus=4,
    logging_steps=25,
    remove_unused_columns=False,  # required as the PeftModel forward doesn't have the signature of the wrapped model's forward
    label_names=["labels"],  # same reason as above
    # max_source_length=4480,
